<template>
  <li v-if="comment" class="comments">
    ...
    <form @submit.prevent="replyToComment" v-if="isReply">
      <p class="warn" v-if="fillArea">
        Please fill the given area before you write to
        {{ this.comment.userEmail }}
      </p>
      <div class="comments-reply">
        <textarea
          class="comments-reply__text"
          name="reply-comment"
          cols="30"
          rows="5"
...
export default {
  ...
  methods: {
    ...
    replyToComment() {
      let repliesSend = db.ref(
        `feedbacks/${this.feedbackKey}/${this.comment.keys}`
      );
